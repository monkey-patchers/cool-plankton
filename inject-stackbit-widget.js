...
if (!publishDir || !stackbitWidgetUrl) {
    throw new Error(
        `[inject-stackbit-widget.js] command must be called with two  arguments specifying the location of publish dir and the widget url`
    );
}
...
findFilesInDir(publishDir, 'html').forEach(file=>{
    ...
    pageCount++;
    injectHtmlBeforeHtml(file, "</body>", `<script src="${stackbitWidgetUrl}"></script>`);
});
